...
public class App {
	...
	public void main(String strXMLFile, String csvFilePath, @Optional("Optional parameter") String proxy)
		...
		try {
			...
			boolean optionalStart1 = false;
			boolean optionalClose1 = false;
				...
				try {
					...
					while (element != null) {
						if ("optionalstart".equalsIgnoreCase(element)) {
							...
							if ("optionalstart".equalsIgnoreCase(element)) {
								LoggerUtil.debug("OptionalSClose Tag is Mandatory with OptionalStart Tag");
								continue;
							}
							element1 = element;
							while (element1 != null) {
								if ("optionalstart".equalsIgnoreCase(element1) || "logout".equalsIgnoreCase(element1)) {
									LoggerUtil.debug("OptionalClose Tag is Mandatory with OptionalStart Tag");
									break;
								}
								if ("optionalclose".equalsIgnoreCase(element1)) {
									break;
								}
								element1 = testCase1.getNextElement();
							}
						}
						if ("optionalclose".equalsIgnoreCase(element)) {
							optionalStart = false;
							optionalStart1 = false;
							String msg = testCase.getFileName() + "::Line-> " + testCase.getLineNumber() + " => "
									+ suiteName + " => " + testCaseName + " => <" + element + "> Processing";
							System.out.println(msg);
							LoggerUtil.debug(msg);

							if (optionalClose1 == false) {
								LoggerUtil.debug("OptionalStart Tag is Mandatory with OptionalClose Tag");
							}

							element = testCase.getNextElement();
							optionalClose1 = false;
							if ("optionalstart".equalsIgnoreCase(element)
									|| "optionalclose".equalsIgnoreCase(element)) {
								continue;
							}
						}
							...
							while (executionCnt <= iRetryInterval) {
								try {
									...
									if ("include".equalsIgnoreCase(element)) {
										...
										try {
											this.handler.processElement(element, testCase, csvUtilObj);
										} catch (Exception ex) {
					...
					if (optionalStart1) {
						LoggerUtil.debug("OptionalClose is Mandatory with OptionalStart Tag");
					}
			...
			this.handler.emptyLnkLstOfIfStack();
		}
		...
		try {
			...
			while (element != null) {
				if ("endpost".equalsIgnoreCase(element)) {
					...
					this.handler.processElement(element, xmlUtilObj, csvUtilObj);
				} else {
