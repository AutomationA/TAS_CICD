...
//////////////////////////////////////////////////////////
			String strWait1 = attributeMap.get(Constants.ATTRIBUTE_WAIT);
			String finalStrWait1 = getTimeOut(strWait1);
			LoggerUtil.debug("timeout : " + finalStrWait1);
			HashMap<String, String> attributeMapForUseDiffAcc = new HashMap<String, String>() {
 
				{
					put(Constants.LOCATOR_TYPE, "xpath");
					put(Constants.LOCATOR_EXPRESSION, "//div[@class='mectrl_truncate' and text()='Sign in with a different account']");
					put(Constants.ATTRIBUTE_CRITERIA, "found");
					put(Constants.ATTRIBUTE_WAIT, finalStrWait1);
				}
			};
			HashMap<String, String> attributeMapForPresentUserCheck = new HashMap<String, String>() {
 
				{
					put(Constants.LOCATOR_TYPE, "xpath");
					put(Constants.LOCATOR_EXPRESSION, "//div[@aria-label='Sign in with "+strUserName+" work or school account.']");
					put(Constants.ATTRIBUTE_CRITERIA, "found");
					put(Constants.ATTRIBUTE_WAIT, finalStrWait1);
				}
			};
			HashMap<String, String> attributeMapForUseAnotherAcc = new HashMap<String, String>() {
 
				{
					put(Constants.LOCATOR_TYPE, "xpath");
					put(Constants.LOCATOR_EXPRESSION, "//div[@id='otherTileText' and text()='Use another account']");
					put(Constants.ATTRIBUTE_CRITERIA, "found");
					put(Constants.ATTRIBUTE_WAIT, finalStrWait1);
				}
			};
			HashMap<String, String> attributeMapForCnfmAcc = new HashMap<String, String>() {
				
				{
					put(Constants.LOCATOR_TYPE, "xpath");
					put(Constants.LOCATOR_EXPRESSION, "//div[@data-test-id="+strUserName+"]");
					put(Constants.ATTRIBUTE_CRITERIA, "found");
					put(Constants.ATTRIBUTE_WAIT, finalStrWait1);
				}
			};
			HashMap<String, String> attributeMapForClickWindowElement = new HashMap<String, String>(){
				 
				{
					put(Constants.IMAGE_PATH,"C:\\Project Code1\\TAS_3D_12\\03_Source_Code\\Development\\TestAutomationProject\\testsuites\\OutlookLogin\\TC_112_12\\no_thanks_.png" );
					put(Constants.ATTRIBUTE_CRITERIA, "found");
					put(Constants.ATTRIBUTE_WAIT, finalStrWait1);
				}
			};
			/*
			 * if(library.ifCondition(driver, attributeMapForClickWindowElement,
			 * finalStrWait1)) { WebElement cc=driv }
			 */
			if (library.ifCondition(driver, attributeMapForUseDiffAcc, null)) {
 
				WebElement SignWithDiffAcc = driver.findElement(By.xpath("//div[@class='mectrl_truncate' and text()='Sign in with a different account']"));
				driver.click(SignWithDiffAcc);
			}
			if (library.ifCondition(driver, attributeMapForPresentUserCheck, null)) {
 
				WebElement SignWithDiffAcc = driver.findElement(By.xpath("//div[@aria-label='Sign in with "+strUserName+" work or school account.']"));
				driver.click(SignWithDiffAcc);
			}
			else if (library.ifCondition(driver, attributeMapForUseAnotherAcc, null)) {
 
				WebElement SignWithDiffAcc = driver.findElement(By.xpath("//div[@id='otherTileText' and text()='Use another account']"));
				driver.click(SignWithDiffAcc);}
			if (library.ifCondition(driver, attributeMapForCnfmAcc, null)) {
				
				WebElement SignWithDiffAcc = driver.findElement(By.xpath("//div[@data-test-id="+strUserName+"]"));
				driver.click(SignWithDiffAcc);
			}
			/////////////////////////////////////////////////////
			...
			try {
				wbusername = driver.findElement(By.xpath("//div/button[@type='submit']"));
			} catch (Exception e) {
			...
			if (library.ifCondition(driver, attributeMapForUserCheck, null)) {
				...
				WebElement wbForExistAccount = driver.findElement(By.xpath("//small[contains(text(),'" + strUserName + "')]"));
				...
				library.openURL(driver, new HashMap<String, String>() {
{
	...
	public void searchOutlookMail(Driver driver, Map<String, String> attributeMap) throws Exception {
		...
		List<WebElement> lstWbElementForSearchText = driver.findElements(By.xpath("//div[@id='groupHeaderTop results']//parent::div//div"));
		...
		for (int iIndex = 0; iIndex < lstWbElementForSearchText.size(); iIndex++) {
			if (lstWbElementForSearchText.get(iIndex).getAttribute("id").equalsIgnoreCase("groupHeaderTop results")) {
				...
				if (lstWbElementForSearchText.get(iIndex + 1).getAttribute("aria-label").contains(strSearchMailId)) {
					...
					throw new AssertionException("Please provide valid email id for search text.");
				}
			}
		}
		...
		if (!blForAllResults) {
			WebElement wbSearchElement = driver.findElement(lstWbElementForSearchText.get(iIndexForTopResult + 1),
					By.xpath("//span[@title='"+strSearchMailId+"']"));
	...
	public void assertOutlookMailHeader(Driver driver, Map<String, String> attributeMap) throws AssertionException {
		...
		if (null != strSubject) {
			try {
				...
				driver.findElement(By.xpath("//span[@title='"+ strSubject +"']"));
