...
public class Driver {
	...
	/**
	 * method to set the attribute call value
	 * 
	 * @author Steepgraph Systems
	 * @param WebElement
	 * @param color
	 ...
	 * @throws Exception
	 */
	public void setColor(WebElement wbElement, String color) throws Exception {
		JavascriptExecutor executor = (JavascriptExecutor) webDriver;
		executor.executeScript("arguments[0].style.color = '" + color + "';", wbElement);
	}
	 ...
	 * @throws Exception
	 */
	public void clickOverrideLink(String strBrowser) throws Exception {
		...
		try {
			if (!strBrowser.equalsIgnoreCase("firefox")) {
				...
				wbOverrideLink = this
						.findElement(By.xpath("//a[contains(text(),'I understand the risk, but want to proceed')]"));
				this.click(wbOverrideLink);
			}
		} catch (Exception e) {
